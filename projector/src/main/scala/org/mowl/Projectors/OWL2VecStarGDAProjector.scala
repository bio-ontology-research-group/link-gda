    ...
    val equivalenceTriples = equivalenceAxioms.flatMap(
      x => {
        ...
        superClass.getClassExpressionType.getName match{
          ...
          case _ => {
            //check if subclass is an HP class
            val isHPSubClass = subClass.isInstanceOf[OWLClass] &&
              subClass.asInstanceOf[OWLClass].toStringID.startsWith("http://purl.obolibrary.org/obo/HP_")
            //check if superclass contains a GO class or a UBERON class
            val superClassIRIs = superClass.getClassesInSignature.asScala.map(_.toStringID).toList
            val containsGOorUBERON = superClassIRIs.exists(iri =>
              iri.startsWith("http://purl.obolibrary.org/obo/GO_") ||
              iri.startsWith("http://purl.obolibrary.org/obo/UBERON_"))
            // if both conditions are satisfied, then proceed with printing the axiom and return nil. Otherwise just return nil without printing the axiom
            if (isHPSubClass && containsGOorUBERON) {
              println("Equivalence axiom not handled. \nSubclass: " + subClass + "\nSuperclass: " + x)
            }
            Nil
          }
        }
    ...
    if (superClassType != "Class") {
	  ...
	  val subClass_ = lift2QuantifiedExpression(subClass)
          projectQuantifiedExpression(subClass_, ontology) match {
            ...
            case None => {
              val isHPSubClass = subClass.getClassesInSignature.asScala.exists(_.toStringID.startsWith("http://purl.obolibrary.org/obo/HP_"))
              val superClassIRIs = superClass.getClassesInSignature.asScala.map(_.toStringID).toList
              val containsGOorUBERON = superClassIRIs.exists(iri =>
                iri.startsWith("http://purl.obolibrary.org/obo/GO_") ||
                iri.startsWith("http://purl.obolibrary.org/obo/UBERON_"))
              if (isHPSubClass && containsGOorUBERON) {
                println("---------------------------------")
                println("SubClass axiom with complex subclass not handled: \nSubclass:" + subClass + "\nSuperclass" + superClass)
                println("---------------------------------")
              }
              Nil
            }
        ...
        case _ => {
          val isHPSubClass = subClass.getClassesInSignature.asScala.exists(_.toStringID.startsWith("http://purl.obolibrary.org/obo/HP_"))
          val containsGOorUBERON = superClass_.toStringID.startsWith("http://purl.obolibrary.org/obo/GO_") ||
            superClass_.toStringID.startsWith("http://purl.obolibrary.org/obo/UBERON_")
          if (isHPSubClass && containsGOorUBERON) {
            println("---------------------------------")
            println("SubClass axiom with distinct quantifier not handled. \nSubclass: " + subClass + "\nSuperclass: " + superClass_)
            println("----------------------------------")
          }
          Nil
        }
              ...
              if (isHPSubClass) {
                ...
                if (nestedTriples.nonEmpty) {
                  ...
                  val containsGOorUBERON = superClassIRIs.exists(iri =>
                    iri.startsWith("http://purl.obolibrary.org/obo/GO_") ||
                    iri.startsWith("http://purl.obolibrary.org/obo/UBERON_"))
                  if (containsGOorUBERON) {
                    println("SubClass axiom with complex superclass not handled: \nSubclass:" + subClass + "\nSuperclass: " + superClass)
                  }
                  Nil
                }
              } else Nil
  ...
  def processNestedSomeValuesFrom(subClass: OWLClass, superClass: OWLClassExpression): List[Triple] = {
    superClass.getClassExpressionType.getName match {
      case "ObjectSomeValuesFrom" => {
        ...
        val outerRelURI = outerExpr.getProperty.getNamedProperty.toStringID
        val outerRelLocal = outerRelURI.split("/").last
        extractFromExpression(subClass, outerExpr.getFiller, outerRelURI, List(outerRelLocal))
      }
      case _ => Nil
    }
  }

  def extractFromExpression(subClass: OWLClass, expr: OWLClassExpression, outerRelURI: String, relPath: List[String]): List[Triple] = {
    expr.getClassExpressionType.getName match {
      ...
      case "ObjectIntersectionOf" => {
        ...
        operands.flatMap(op => extractFromExpression(subClass, op, outerRelURI, relPath))
      }
      case "ObjectSomeValuesFrom" => {
        val someExpr = lift2QuantifiedExpression(expr)
        val relLocal = someExpr.getProperty.getNamedProperty.toStringID.split("/").last
        extractFromExpression(subClass, someExpr.getFiller, outerRelURI, relPath :+ relLocal)
      }
      case _ => Nil
    }
  }
