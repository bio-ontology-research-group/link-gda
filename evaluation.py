...
def evaluate_2p_model_old(model, test_disease_genes, gene2pheno, gene2function, gene2site, disease2pheno,
                      eval_genes, triples_factory=None,
                      entity_to_id=None, relation_to_id=None,
                      use_phenotypes=True, use_functions=True,
                      use_site=True, output_file_prefix=None,
                      verbose=False):
    """
    Evaluate the model on a given test set using the 2p (two-path) approach.

    Scores each (gene, disease) pair by scoring (gene, indirectly_causes, symptom)
    triples directly via the model's scoring function for all disease symptoms,
    then aggregating via BMA / BMM.

    Args:
        model: The trained KGE model (TransD)
        test_disease_genes: DataFrame with 'Disease' and 'Gene' columns
        gene2pheno: Unused (kept for API compatibility)
        gene2function: Unused (kept for API compatibility)
        gene2site: Unused (kept for API compatibility)
        disease2pheno: Dictionary mapping diseases to phenotypes
        eval_genes: List of genes to evaluate
        ...
        use_phenotypes: Unused (kept for API compatibility)
        use_functions: Unused (kept for API compatibility)
        use_site: Unused (kept for API compatibility)
        output_file_prefix: Optional prefix for output files. If None, results are not saved.
        verbose: Whether to print detailed results.
    ...
    indirectly_causes_id = relation_to_id['indirectly_causes']

    gene_to_index = {gene: i for i, gene in enumerate(eval_genes)}
    num_genes = len(eval_genes)

    gene_ids = th.tensor([entity_to_id[g] for g in eval_genes])  # (num_genes,)
    ...
    with tqdm(total=len(test_pairs), desc='Evaluating', leave=False) as pbar:
        for test_disease, test_gene in test_pairs:
            ...
            disease_phenos = disease2pheno[test_disease]
            pheno_ids = th.tensor([entity_to_id[p] for p in disease_phenos])  # (L,)
            L = len(pheno_ids)

            # ---------------------------------------------------------- #
            # Build HRT tensor for all (gene, indirectly_causes, symptom) #
            # combinations: shape (num_genes * L, 3).                     #
            # ---------------------------------------------------------- #
            h = gene_ids.unsqueeze(1).expand(num_genes, L).reshape(-1)       # (num_genes * L,)
            t = pheno_ids.unsqueeze(0).expand(num_genes, L).reshape(-1)      # (num_genes * L,)
            r = th.full((num_genes * L,), indirectly_causes_id, dtype=th.long)

            hrt = th.stack([h, r, t], dim=1)  # (num_genes * L, 3)

            with th.no_grad():
                scores = model.score_hrt(hrt).cpu().view(num_genes, L)  # (num_genes, L)

            # Gene-centric: best-matching symptom per gene
            gene_centric = scores.max(dim=1).values   # (num_genes,)

            # Disease-centric: for each symptom, best-matching gene, averaged
            disease_centric = scores.mean(dim=1)      # (num_genes,)

            bma_scores = scores.mean(dim=1)
            bmm_scores = scores.max(dim=1).values
            # bma_scores = (gene_centric + disease_centric) / 2
            # bmm_scores = th.max(gene_centric, disease_centric)

            bma_results.append((test_gene, test_disease, gene_to_index[test_gene], bma_scores.tolist()))
            bmm_results.append((test_gene, test_disease, gene_to_index[test_gene], bmm_scores.tolist()))
...
def evaluate_2p_model(model, test_disease_genes, gene2pheno, gene2function, gene2site, disease2pheno,
                       eval_genes, triples_factory=None, entity_to_id=None, relation_to_id=None,
                       use_phenotypes=True, use_functions=True, use_site=True,
                       output_file_prefix=None, verbose=False):
    """
