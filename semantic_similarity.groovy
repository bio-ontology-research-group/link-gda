...
logger.info("disease2pheno size: ${disease2pheno.size()}")
logger.info("disease2pheno example: ${disease2pheno.take(1)}")
	...
	try 
{
	...
	def test_gene_index = evalGenes.indexOf(test_gene)
	    if (test_gene_index == -1) {
		logger.info("Test gene ${test_gene} not found in evalGenes list")
	    }
